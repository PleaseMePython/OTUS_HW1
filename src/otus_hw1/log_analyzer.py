...
def get_log_file_name(path: str) -> FileInfo:
    ...
    pattern = re.compile(r"^nginx-access-ui\.log-" +
                         r"2\d\d\d" +               # Год
                         r"((0[1-9])|(1[0-2]))" +   # Месяц
                         r"(([0-2]\d)|(3[0-1]))" +  # День
                         r"(\.gz){0,1}$")           # Расширение
    # Список файлов (исключая папки) по заданной маске
    file_list = [s for s in os.listdir(path=path)
                 if pattern.match(s) and
    ...
    for file_name in file_list:
        ...
        # проверкой по маске
        file_date = date(year=int(file_name[20:24]), month=int(
            file_name[24:26]), day=int(file_name[26:28]))
        # Сохраняем имя файла, если дата в имени больше сохраненной
        if file_date > result.f_date:
            result = FileInfo(name=os.path.join(path, file_name),
                              f_date=file_date,
...
def get_config(default_config: Dict) -> Dict:
    ...
    cnf_parser.read(os.path.abspath(cnf_file_name),encoding='utf-8')
    return dict(cnf_parser["DEFAULT"])
...
def extract_url(log_line: str) -> str:
    ...
    url_off_end = log_line.find(" ", url_off_start + url_start_shift) \
        if url_off_start != -1 else -1
...
def parse_log_file(file_info: FileInfo) -> UrlInfo:
    """Парсинг файла лога.

    :arg file_info - кортеж с именем и датой файла
    :return: Путь к папке
    """
    err_logger = structlog.get_logger()
    if not os.access(file_info.name,mode=os.R_OK):
        err_logger.error("Нет права доступа к файлу лога", path=file_info.name)
        return

    with (open(file_info.name, 'r', encoding='utf-8')
          if not file_info.is_archive
          else gzip.open(file_info.name, 'rt', encoding='utf-8') as file):
        for logLine in file:
            yield UrlInfo(url=extract_url(logLine),
                          requestTime=extract_time(logLine))
...
def process_log(file_attr: FileInfo) -> LogInfo:
    ...
    for url_info in parse_log_file(file_attr):
        ...
        log_grouped.setdefault(
            url_info.url, []).append(
            url_info.requestTime)

    return LogInfo(url_count=url_count,
                   url_total_time=url_total_time,
...
def round_floats(src):
    ...
    """
    if isinstance(src, float): return round(src, 3)
    if isinstance(src, dict): return {k: round_floats(v) for k, v in src.items()}
    if isinstance(src, (list, tuple)): return [round_floats(x) for x in src]
    return src


def get_report_file_name(report_dir: str,
                 file_date: date) -> str:
    ...
    return os.path.join(os.path.abspath(report_dir), rep_file_name)


def write_report(url_stats: UrlStatsTab,
    ...
    if not os.access(template_file_name,mode=os.R_OK):
        err_logger.error("Нет права доступа к файлу шаблона", path=template_file_name)
        return
    if not os.access(os.path.dirname(report_file_name),mode=os.W_OK):
        err_logger.error("Нет права доступа к папке отчетов", path=report_file_name)
        return

    src_for_json = []
    for url_line in url_stats:
        src_for_json.append(url_line._asdict())
...
def analyse_logs(actual_config: Dict) -> None:
    ...
    err_logger = structlog.get_logger()
    # Имя последнего лога
    file_attr = get_log_file_name(os.path.abspath(actual_config["log_dir"]))
    report_file_name = get_report_file_name(actual_config["report_dir"], file_attr.f_date)
    if file_attr.name == "":
        # Ничего не нашли
        err_logger.error("Log file fot found")
    elif os.path.exists(report_file_name):
        ...
        url_stats = gather_stats(
            log_grouped, int(
                actual_config["report_size"]))
        write_report(url_stats,
                     actual_config["report_template"],
                     report_file_name)
...
def setup_err_log(actual_config: Dict) -> Any:
    ...
    """
    try:
        error_path = actual_config["error_file"]
    except KeyError:
        error_path = ""

    # Путь указан неверно
    wrong_path = ( os.path.exists(error_path) and
                   not os.path.isfile(error_path))
